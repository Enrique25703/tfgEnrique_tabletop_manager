...
<head>
  ...
  <style>
    ...
    .assistant-toggle{position:fixed;right:24px;bottom:24px;z-index:61;border-radius:999px;box-shadow:0 12px 34px rgba(0,0,0,.42)}
    .assistant-panel{position:fixed;right:24px;bottom:84px;z-index:60;display:flex;flex-direction:column;width:min(420px,calc(100vw - 32px));max-height:min(650px,calc(100vh - 120px));overflow:hidden;border:1px solid var(--line-strong);border-radius:20px;background:rgba(9,16,26,.98);box-shadow:0 24px 70px rgba(0,0,0,.58)}
    .assistant-panel[hidden]{display:none}.assistant-header{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;padding:16px;border-bottom:1px solid var(--line)}.assistant-header h2{margin:0;font-size:1.05rem}.assistant-header p{margin:5px 0 0;color:var(--muted);font-size:.84rem}.assistant-close{padding:6px 9px}
    .assistant-messages{display:grid;gap:12px;min-height:180px;padding:16px;overflow:auto}.assistant-message{max-width:92%;padding:11px 13px;border-radius:15px;line-height:1.45;white-space:pre-wrap}.assistant-message.user{justify-self:end;background:rgba(68,112,167,.72)}.assistant-message.bot{justify-self:start;background:rgba(31,46,67,.96)}.assistant-message.error{border:1px solid #ad4f59;color:#ffb1b8}.assistant-message ul{margin:8px 0 0;padding-left:20px}.assistant-message strong{display:block;margin-top:10px}
    .assistant-form{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:9px;padding:14px;border-top:1px solid var(--line)}.assistant-form input{min-width:0}.assistant-status{grid-column:1/-1;min-height:1.2em;margin:0;color:var(--muted);font-size:.82rem}
    ...
    <div class="app-main">
      ...
      <main class="page-content">
        <div id="creadorListaApp" data-formato-juego="<c:out value='${creadorLista.formatoJuego}'/>" data-nombre-lista="<c:out value='${creadorLista.nombreLista}'/>" data-faccion="<c:out value='${creadorLista.faccion}'/>" data-ejercito="<c:out value='${creadorLista.ejercito}'/>" data-limite-puntos="<c:out value='${creadorLista.limitePuntos}'/>" data-url-guardado="/creador-listas-40k/guardar" data-url-asistente="/creador-listas-40k/asistente/analizar">
          ...
          <button type="button" class="button-primary assistant-toggle" id="abrirAsistente" aria-controls="panelAsistente" aria-expanded="false">Asistente de lista</button>
          <aside class="assistant-panel" id="panelAsistente" aria-label="Asistente de lista" hidden>
            <header class="assistant-header">
              <div><h2>Asistente de lista</h2><p>Analiza las unidades que tienes ahora mismo.</p></div>
              <button type="button" class="assistant-close" id="cerrarAsistente" aria-label="Cerrar asistente">Cerrar</button>
            </header>
            <div class="assistant-messages" id="mensajesAsistente" aria-live="polite">
              <div class="assistant-message bot">Pregúntame cómo mejorar la lista o pulsa Enviar para obtener un análisis general.</div>
            </div>
            <form class="assistant-form" id="formAsistente">
              <input id="preguntaAsistente" maxlength="300" placeholder="¿Cómo puedo mejorar esta lista?" autocomplete="off" />
              <button type="submit" class="button-primary" id="enviarAsistente">Enviar</button>
              <p class="assistant-status" id="estadoAsistente"></p>
            </form>
          </aside>
        </div>
      </main>
